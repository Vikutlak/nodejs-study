...
handlers.index = (data, callback) => {
  ...
  if(data.method == 'get'){
    ...
    const templateData = {
      ...
      'head.description': 'We offer free simple uptime monitoring for HTTP/HTTPS sites of all kinds. When your site goes down we\'ll send you a notification.',
      'body.class': 'index',
    };
...
handlers.favicon = (data, callback) => {
  ...
  if(data.method == 'get'){
    ...
    helpers.getStaticAsset('favicon.ico', (err, data) => {
      if(!err && data){
        // Callback the data
        callback(200, data, 'favicon');  // status, the data, the type of data
      } else {
        callback(500);
      }
    });
    ...
    if(trimmedAssetName.length > 0){
      ...
      helpers.getStaticAsset(trimmedAssetName, (err, data) => {
        if(!err && data){
          ...
          // Callback the data
          callback(200, data, contentType);  // status, the data, the type of data
        } else {
          callback(500);
        }
      });
...
handlers.users = (data, callback) => {
  ...
  if(acceptableMethods.indexOf(data.method) > -1){
    handlers._users[data.method](data, callback); // _users because it is used only for this, not for something outside this function
  } else {
    callback(405);
  }
};
...
handlers.tokens = (data, callback) => {
  const acceptableMethods = ['post', 'get', 'put', 'delete'];
  if(acceptableMethods.indexOf(data.method) > -1){
    handlers._tokens[data.method](data, callback); // _tokens because it is used only for this, not for something outside this function
  } else {
    callback(405);
  }
};
...
handlers.checks = (data, callback) => {
  const acceptableMethods = ['post', 'get', 'put', 'delete'];
  if(acceptableMethods.indexOf(data.method) > -1){
    handlers._checks[data.method](data, callback); // _checks because it is used only for this, not for something outside this function
  } else {
    callback(405);
  }
};
        ...
        _data.read('users', userPhone, (err, userData) => {
          if(!err && userData){
            ...
            if(userChecks.length < config.maxChecks){
              // Create ae5b2xqiklp6mprss88u9 random id for the check
